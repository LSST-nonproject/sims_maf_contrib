...
def mag_error(ap_mag, band, calcm5=0): #apparent magnitude and band
   ...
   if calcm5 == 0: calcm5=m5[band]
   X = 10.**(0.4*(ap_mag-calcm5))
   random_2 = np.sqrt((0.04 - gamma[band])*X + gamma[band]*X*X)
   error_2 = random_2**2. + (0.005)**2.
...
def noise_calc(band):
   ...
   sigma=0.03
   sigma_sys=0.005
   fun = lambda x: sigma_sys**2 + (.04-gamma[band])*10**(0.4*(x-m5[band])) + gamma[band]*10**(0.8*(x-m5[band])) - sigma**2
...
def starcount_bymass(eqRA, eqDEC, m1, m2, band):
   ...
   starcounts = [ y*starcount(eqRA, eqDEC, x[0], x[1]) for x, y in zip(distances, massfractions) ]
   return sum(starcounts)


if __name__ == "__main__":
   print(starcount_bymass(float(sys.argv[1]), float(sys.argv[2]), float(sys.argv[3]), float(sys.argv[4]), sys.argv[5]))
