...
# lixl@udel.edu
# https://github.com/Bricmank/TDEmetric/blob/master/TDEpopulationMetric.ipynb
...
class TDEsMonteMetric(BaseMetric):
    """Based on the transientMetric, but uses an list of ascii input files to choose randomly from, 
    which contains asciifiles of different types of TDEs. Provides an option to write out lightcurve.

    Calculates what fraction of the transients would be detected. Best paired with a spatial slicer.
    ...
    ----------
    asciifilelist : list
        The ascii file list contains ascii files of the inputs for the lightcurve (per filter):
    ...
    def __init__(self, asciifilelist, metricName='TDEsMonteMetric',
        ...
        if asciifilelist is None:
            sims_maf_contrib_dir = os.getenv("SIMS_MAF_CONTRIB_DIR")
            filename = os.path.join(sims_maf_contrib_dir, 'data/tde/TDEfaintfast_z0.1.dat')
            asciifilelist = [filename]
        ...
        if self.dataout:
            ...
            super(TDEsMonteMetric, self).__init__(col=[self.mjdCol, self.m5Col, self.filterCol],
                                                       ...
                                                       metricName=metricName, **kwargs)
        self.read_lightCurve(asciifilelist)

        print('Finish initializing metric')

    def read_lightCurve(self, asciifilelist):

        #if not os.path.isfile(asciifilelist):
        #    raise IOError('Could not find lightcurve ascii file %s' % (asciifilelist))
        lcv_templates = []
        lcv_templates_names = []
        for file in asciifilelist:
            lcv_templates.append(np.genfromtxt(file, dtype=[('ph', 'f8'), ('mag', 'f8'), ('flt', 'S1')]))
            lcv_templates_names.append(file)

        self.lcv_templates = np.asarray(lcv_templates)
        self.lcv_name = np.asarray(lcv_templates_names)

    def make_lightCurve(self, time, filters):

        lcv_templates = self.lcv_templates

        lcMagsAll = []
        for lcv_template in lcv_templates:
    ...
    def run(self, dataSlice, slicePoint=None):
        """"Calculates the detectability of transient with the a lightcurve chosen
        randomly from the specified list of lightcurves.
        ...
        for i, t0_i in enumerate(t0):
            ...
            idx = random.choice(range(len(self.lcv_templates)))
            lcv_template = self.lcv_templates[idx]
            lcv_name = self.lcv_name[idx][:-21]
            ...
            # make light curve
            lcMags = self.make_lightCurve(lcEpoch, lcFilters)[idx]
            ...
            lcDict = {'lcN': i, 'lcMjd': lcMjd, 'lcEpoch': lcEpoch, 'lcFilters': lcFilters,
              ...
              'detected': lcDetect, 'lcName': lcv_name}
