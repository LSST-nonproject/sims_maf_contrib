...
class TransientAsciiSEDMetric(BaseMetric):
    ...
    def run(self, dataSlice, slicePoint=None):
        ...
        -------
        float or dict
            The total number of transients, and fraction of transients, that
            could be detected. (if output_data is False) Otherwise, a dictionary
            with arrays of 'transient_id', 'lcMag', 'detected', 'time',
            'detectThresh', 'filter'
        """
        ...
        survey_duration = np.max([slicer_time_span, self.survey_duration])
        ...
        max_num_transients = 0
        # Set this, in case survey_start was set to be much earlier than this
        # data (so we start counting at 0).
        transient_id_start = -1 * np.floor(
            (dataSlice[self.mjdCol].min() - survey_start) / self.transient_duration
        )
        # Consider each different 'phase shift' separately.
        # We then just have a series of lightcurves, taking place back-to-back.
        for time_shift in time_phase_shifts:
            # Update the maximum possible transients that could have been
            # observed during survey_duration.
            max_num_transients += np.ceil(
                survey_duration / (self.transient_duration / 365.25)
            )
            # Calculate the observation epoch for each transient lightcurve.
            observation_epoch = (
                dataSlice[self.mjdCol] - survey_start + time_shift
            ) % self.transient_duration
            # Identify the observations which belong to each distinct transient.
            transient_id = (
                np.floor(
                    (dataSlice[self.mjdCol] - survey_start) / self.transient_duration
                )
                + transient_id_start
            )
            # Set the starting id number for the next phase shift
            transient_id_start = transient_id.max()
            # Find the set of uniquely observed transients
            unique_transient_id = np.unique(transient_id)
            # Find the starting index for each transient_id.
            transient_start_index = np.searchsorted(
                transient_id, unique_transient_id, side="left"
            )
            # Find the ending index of each transient_id.
            transient_end_index = np.searchsorted(
                transient_id, unique_transient_id, side="right"
            )

            # Generate the actual light curve magnitudes and SNR
            light_curve_mags = self.make_lightcurve(
                observation_epoch, dataSlice[self.filterCol]
            )
            light_curve_SNRs = m52snr(light_curve_mags, dataSlice[self.m5Col])

            # Initilize array for observations below or above SNR threshold
            obs_above_SNR_threshold = np.zeros(len(light_curve_SNRs), dtype=bool)
            ...
            for filt in np.unique(dataSlice[self.filterCol]):
                ...
                obs_above_SNR_threshold[filter_match] = np.where(
                    light_curve_SNRs[filter_match] >= self.detect_SNR[filt], True, False
                )
            ...
            for i, (start_ind, end_ind) in enumerate(
                ...
                detected_light_curve_sections = np.unique(
                    np.floor(
                        observation_epoch_above_thresh
                        / self.transient_duration
                ...
                # detected to False.
                detected_filters = dataSlice[start_ind:end_ind][
                    np.where(obs_above_SNR_threshold[start_ind:end_ind])
                ][self.filterCol]
                if len(np.unique(detected_filters)) < self.num_filters:
                    transient_detected[t_id] = False
                    continue
                ...
                if (self.filter_time is not None) and (self.num_filters > 1):
                    final_filter_detection_ind = np.searchsorted(
                        observation_epoch_above_thresh,
                        observation_epoch_above_thresh + self.filter_time,
                        "right",
                    )
                    final_filter_detection_ind = np.where(
                        final_filter_detection_ind
                        < len(observation_epoch_above_thresh) - 1,
                        final_filter_detection_ind,
                        len(observation_epoch_above_thresh) - 1,
                    )
                    is_detected = False
                    for i, filt_end_ind in enumerate(final_filter_detection_ind):
                        if (
                            len(np.unique(detected_filters[i:filt_end_ind]))
                            >= self.num_filters
                        ):
                            is_detected = True
                            break
                ...
                # (more complicated conditions should go later in the loop,
                # simpler ones earlier).

            # Find the unique number of light curves that passed the required
            # number of conditions
            num_detected += len(np.where(transient_detected == True)[0])
        ...
        if self.output_data:
            ...
            return {
                ...
                "lcMag": light_curve_mags,
                "SNR": light_curve_SNRs,
                "detected": transient_detected,
            }
        else:
            return float(num_detected) / max_num_transients
